...
void SortedMergeRecur(Node*& xPtr, Node*& yPtr, Node*& zPtr){
   if(xPtr == 0 && yPtr == 0) return;
   if(xPtr->link == 0 && yPtr->link == 0) return;
   Node* xCursor = xPtr;
   Node* yCursor = yPtr;
   Node* zCursor = zPtr;

   if(yCursor->data > xCursor->data){
      if(zCursor == 0){
         zCursor = xCursor;
      } else {
         zCursor->link = xCursor;
         zCursor = zCursor->link;
      }
      xCursor = xCursor->link;
   } else if(xCursor->data > yCursor->data){
      if(zCursor->data == 0){
         zCursor = yCursor;
      } else {
         zCursor->link = yCursor;
         zCursor = zCursor->link;
      }
      yCursor = yCursor->link;
   ...
   SortedMergeRecur(xCursor, yCursor, zCursor);
}
